...
First, check sample depths for rarefaction. Minimum sample depth is right around 7k reads, so we will rarefy at 7k.
...
plot_richness(bb16S_bac_rarefy, x = "Treatment", measures = c("Observed"), color = "Site") + geom_boxplot() + ggtitle("Site by treatment")
...
#pairwise with site strata arugment
library(pairwiseAdonis)
pairwise.adonis2(dist ~ Treatment + Timepoint, strata = "Site",  data= sd_adonis, nperm = 10000)
pairwise.adonis2(dist ~ Treatment + Timepoint,  data= sd_adonis, nperm = 10000)
...
#####UNIFRAC MEASURES FOR WHOLE DATASET
...
anova(betadisper(dist.uf, group = sd_adonis$Site))
boxplot(betadisper(dist.uf, group = sd_adonis$Site))
...
dist.ufu<-phyloseq::distance(bb16S_bac_ML, method =  "unifrac")
#full model with no strata argument, essentially the effect of each main effect without controlling for any potential groups. 
adonis2(dist.ufu ~  Site * Treatment * Timepoint, data= sd_adonis, permutations = 1000)
#this model explores the effect of Site, treatment and time point and restricts permutations to time points. 
adonis2(dist.ufu ~  Site * Treatment * Timepoint, data= sd_adonis, strata = sd_adonis$Timepoint, permutations = 1000)
#this model explores the effect of Site, treatment and time point and restricts permutations sites. 
adonis2(dist.ufu ~  Site * Treatment * Timepoint, data= sd_adonis, strata = sd_adonis$Site, permutations = 1000)
...
dist.ufu<-phyloseq::distance(bb16S_bac_ML_B, method =  "wunifrac")
#full model with no strata argument, essentially the effect of each main effect without controlling for any potential groups. 
adonis2(dist.ufu ~  Site * Treatment,  data= sd_adonis, permutations = 1000)
#this model explores the effect of Site, treatment and time point and restricts permutations sites. 
adonis2(dist.ufu ~  Site * Treatment, data= sd_adonis, strata = sd_adonis$Site, permutations = 1000)
```
...
efaidnbmnnnibpcajpcglclefindmkaj/<https://john-quensen.com/wp-content/uploads/2017/12/Ordiplots_with_ggplot.pdf>
...
ord_site <- gg_ordiplot(ord_cap, groups = sd_adonis$Site, scaling = 1, choices = c(1, 2), kind = c( "se"), conf = 0.95, show.groups = "all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, plot = TRUE) 
ord_site_plot <- ord_site$plot
ord_site_plot <- ord_site_plot + theme_classic() + guides(color=guide_legend(title="Site"))


ord_cap_w<-ordinate(physeq = bb16S_bac_ML, method = "CAP", distance = "wunifrac", formula = ~  Site + Timepoint )
w_cap_plot<-plot_ordination(physeq = bb16S_bac_ML, ordination = ord_cap_w, type = "Samples", color = "Site", shape = "Timepoint" ) + theme_classic()  + ggtitle("Weighted Unifrac CAP")  + theme(plot.title = element_text(hjust = 0.5))
...
ord_cap_uw<-ordinate(physeq = bb16S_bac_ML, method = "CAP", distance = "unifrac", formula = ~  Site + Timepoint )
uw_cap_plot<-plot_ordination(physeq = bb16S_bac_ML, ordination = ord_cap_uw, type = "Samples", color = "Site", shape = "Timepoint" ) + theme_classic()  + ggtitle("Unweighted Unifrac CAP")  + theme(plot.title = element_text(hjust = 0.5))
#strongest grouping by Site. Time point also has a smaller effect and the groupings are not nearly as clear. There are several outlier samples that ordinate far to the right on the CAP1 axis. 
...
gg_ordiplot(ord_cap, groups, scaling = 1, choices = c(1, 2), kind = c("sd", "se", "ehull"), conf = NULL, show.groups = "all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, plot = TRUE)
```
...
ord_site <- gg_ordiplot(ord_nmds, groups = sd_adonis$Site, scaling = 1, choices = c(1, 2), kind = c( "se"), conf = 0.95, show.groups = "all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, plot = TRUE) 
ord_site_plot <- ord_site$plot
ord_site_plot <- ord_site_plot + theme_classic() + guides(color=guide_legend(title="Site"))  + xlab("NMDS1") + ylab("NMDS2")
ord_site_plot 
ggsave("../../Figures/Figure3_NMDS_Site.pdf")

#weighted Unifrac
ord_nmds<-ordinate(physeq = bb16S_bac_ML, method = "NMDS", distance = "wunifrac")
ord_nmds$stress
plot_ordination(physeq = bb16S_bac_ML, ordination = ord_nmds, type = "Samples", color = "Site", shape = "Treatment" ) + theme_classic() + ggtitle("Weighted Unifrac NMDS")
...
ord_nmds$stress
plot_ordination(physeq = bb16S_bac_ML, ordination = ord_nmds, type = "Samples", color = "Site", shape = "Treatment" ) + theme_classic() + ggtitle("Unweighted Unifrac NMDS")
```
...
```{r}
TpA <- plot_bar(physeq = subset_samples(bb16S_bac_rarefy, Timepoint == "A"), x = "Site", fill = "Order") + scale_fill_manual(values = col_vector) +geom_bar(stat="identity")  + theme_classic()
TpA

leg <- get_legend(TpA)
plot(leg)

ggsave("../../Figures/Figure2_site_legend.pdf", width = 9)


TpA_nL <- TpA + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 45, hjust = 1))  + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))
TpA_nL
ggsave( "../../Figures/Figure2_SiteTPA.pdf", width = 8)

TpB <- plot_bar(physeq = subset_samples(bb16S_bac_rarefy, Timepoint == "B"), x = "Site", fill = "Order") + scale_fill_manual(values = col_vector)  +geom_bar(stat="identity")  +theme_classic()+ theme(legend.position = "none")  + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
TpB
ggsave("../../Figures/Figure2_SiteTPB.pdf", width = 8)

######
TpA <- plot_bar(physeq = subset_samples(bb16S_bac_rarefy, Timepoint == "A"), x = "Sample", fill = "Order") + scale_fill_manual(values = col_vector) +geom_bar(stat="identity")  + theme_classic() + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))

TpA_nL <- TpA + theme(legend.position = "none") + theme(axis.text.x = element_text(angle = 45, hjust = 1))  + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))
TpA_nL
ggsave( "../../Figures/Figure2_TPA.pdf", width = 8)

TpB <- plot_bar(physeq = subset_samples(bb16S_bac_rarefy, Timepoint == "B"), x = "Sample", fill = "Order") + scale_fill_manual(values = col_vector) +  geom_bar(stat="identity")  +theme_classic()+ theme(legend.position = "none") + theme(axis.text.x =element_text(angle = 45, hjust = 1))  + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))
TpB
ggsave("../../Figures/Figure2_TPB.pdf", width = 8)
...
#genus - 17 members
#asvs - 13 members
core_genus<-taxa_core(phyloseq_obj = bb16S_bac_rarefy_tax_genus, treatment = "Site", frequency = 1, abundance_threshold = 0.00001 )
core_asv<-taxa_core(phyloseq_obj = bb16S_bac_rarefy, treatment = "Site", frequency = 1, abundance_threshold = 0.00001 ) 
...
core_genera_plotA <- plot_bar(subset_samples(core_genus, Timepoint == "A"), x = "Treatment_Site", fill = "Genus") + scale_fill_manual(values = col_vector) +geom_bar(stat="identity") + ggtitle("Core Genera Pre-Treatment") + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12)) + theme(plot.title = element_text(size=15)) + xlab("Treatment by Site") + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))



core_genera_plotA_nl <- core_genera_plotA + theme(legend.position = "none")
ggsave("../../Figures/Figure4_core_tax_sitextreatment_TPA.pdf", width = 9)



core_genera_plotB<-plot_bar(subset_samples(core_genus, Timepoint == "B"), x = "Treatment_Site", fill = "Genus") + scale_fill_manual(values = col_vector) +geom_bar(stat="identity") + ggtitle("Core Genera Post-Treatment") + theme_classic() + theme(plot.title = element_text(hjust = 0.5)) + theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12)) + theme(legend.position = "none") + theme(plot.title = element_text(size=15)) + xlab("Treatment by Site")  + theme(axis.title.x = element_text(size=15)) + theme(axis.title.y = element_text(size=15))



ggsave("../../Figures/Figure4_core_tax_sitextreatment_TPB.pdf", width = 9)



#plot_bar(core_asv, x = "Region_Site", fill = "Genus") + scale_fill_manual(values = col_vector) +geom_bar(stat="identity")


#plot_bar(core_asv, x = "Treatment_Timepoint", fill = "Genus") + scale_fill_manual(values = col_vector) +geom_bar(stat="identity")
...
summary(sample_sums(core_genus)/1100)
sd(sample_sums(core_genus)/1100)
hist(sample_sums(core_genus)/1100, breaks = 20, main = "Percent of total reads \n accounted for by the core", xlab = "Proportion")
...
ord_nmds<-ordinate(physeq = subset_samples(core_genus, Timepoint=="B"), method = "NMDS", distance = "bray", k = 3, trymax = 1000)
ord_nmds$stress
plot_ordination(physeq = core_genus, ordination = ord_nmds, type = "Samples", color = "Site", shape = "Timepoint" ) + theme_classic() + ggtitle("Bray-Curtis NMDS")
#strongest grouping by Site. Time point also has a smaller effect, but these groupings are more clear than in the CAP ordination. 

ord_site <- gg_ordiplot(ord_nmds, groups = data.frame(sample_data(subset_samples(core_genus, Timepoint=="B")))$Treatment, scaling = 1, choices = c(1, 2), kind = c( "se"), conf = 0.95, show.groups = "all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, plot = TRUE) 
ord_site_plot <- ord_site$plot
ord_site_plot <- ord_site_plot + theme_classic() + guides(color=guide_legend(title="Treatment"))  + xlab("NMDS1") + ylab("NMDS2")
ord_site_plot 
#ggsave("../../Figures/Figure3_NMDS_Site.pdf")
```
...
Taxonomic composition of core: Phylum

Actinobacteriota Bacteroidota Firmicutes Proteobacteria 2 1 7 17 Class

Actinobacteria Alphaproteobacteria Bacilli Bacteroidia Clostridia Gammaproteobacteria 2 7 5 1 2 10
...
pairwise.adonis2(dist ~ Treatment, strata = "Site",  data= sd_adonis, nperm = 10000)

#timepoint2 ordination for non-core
ord_nmds<-ordinate(physeq = noncore_asv_t2, method = "NMDS", distance = "bray", k = 3, trymax = 1000)
ord_nmds$stress
plot_ordination(physeq = noncore_asv_t2, ordination = ord_nmds, type = "Samples", color = "Site", shape = "Timepoint" ) + theme_classic() + ggtitle("Bray-Curtis NMDS")
#strongest grouping by Site. Time point also has a smaller effect, but these groupings are more clear than in the CAP ordination. 

ord_site <- gg_ordiplot(ord_nmds, groups = data.frame(sample_data(noncore_asv_t2))$Treatment, scaling = 1, choices = c(1, 2), kind = c( "se"), conf = 0.95, show.groups = "all", ellipse = TRUE, label = FALSE, hull = FALSE, spiders = FALSE, plot = TRUE) 
ord_site_plot <- ord_site$plot
ord_site_plot <- ord_site_plot + theme_classic() + guides(color=guide_legend(title="Treatment"))  + xlab("NMDS1") + ylab("NMDS2")
ord_site_plot 
```
...
#genera
#155 genera are unique to one region. 163 genera are unique to a site.  
...
#asv
#620 asvs are unique to one region. 608 asvs are unique to a site. This means that only 26 asvs unique to a region are not shared for the sites within that region. 
...
#percentage of reads accounted for by core. 
summary(sample_sums(unique_genera_to_site_tax_ps)/1100)
sd(sample_sums(unique_genera_to_site_tax_ps)/1100)

summary(sample_sums(unique_genera_to_region_tax_ps)/1100)
...
baruta_aceto<-baruta_phyloseq(physeq = acetobacter, GroupingVar = "Treatment")
...
#DESEQ
bb16S_bac_Rare_B<-subset_samples(bb16S_bac_rarefy, Timepoint == "B")
aceto_df_rare <- subset_taxa(bb16S_bac_Rare_B, Family =="Acetobacteraceae")
aceto_df_rare <- filter_taxa(aceto_df_rare, function(x) sum(x) > 25, TRUE)
```
...
mod<-aov(OTU_5 ~ Treatment, data = merged_dat)
summary(mod)
shapiro.test(mod$residuals)
plot(residuals(mod))
boxplot(merged_dat$OTU_5 ~ merged_dat$Treatment)


#baruta for individual taxa
baruta_lacto<-baruta_phyloseq(physeq = lactobac, GroupingVar = "Treatment")
...
for(i in 1:length(names(data.frame(otu_table(lactobac))))){
  mod<-aov(unlist(lacto_df[i]) ~ lacto_df$Treatment)
  summary(mod)
  print(summary(mod))
